...
export default function SavedSessionDetail() {
  ...
  const { getSavedSession, deleteSavedSession } = useSession();
  ...
  }, [session?.summary.transcript, session?.turns]);
  ...
  const materials = session.config.attachments ?? [];
  const templateSections = session.scenario.summaryTemplate?.sections ?? [];
  const summarySections = session.summary.sections?.length
    ? session.summary.sections
        ...
        .map((section) => {
          if (
            session.scenario.id === "difficult-conversation" &&
            ...
            if (section.key === "impression") {
              return {
                ...
                traits: [],
                text:
                  session.summary.overview || coachTakeaway(session.summary.wins),
              };
            }
    ...
    : defaultSummarySections(
        session.summary.wins,
        session.summary.drills,
        templateSections,
        materials
      );

  const recapText =
    session.summary.intro || coachTakeaway(session.summary.wins);
      ...
      <View style={{ flex: 1, gap: 16 }}>
        ...
        <View style={{ gap: 10 }}>
          ...
          <Text style={{ color: colors.muted, fontSize: 12, lineHeight: 18 }}>
            {session.config.userRole} • {session.config.partnerStyle}
            {materials.length
        ...
        <View style={{ gap: 10, paddingTop: 4 }}>
          <AppButton
            ...
            onPress={() => {
              Alert.alert(
                "Delete recording?",
                "This will permanently remove the saved session from history.",
                [
